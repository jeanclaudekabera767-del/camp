...
export const Route = createFileRoute("/cart")({
  head: () => ({
    meta: [
      { title: "Votre panier — CampVan" },
      { name: "description", content: "Vérifiez et validez vos achats de camping-cars." },
    ],
  }),
  component: CartPage,
});
      ...
      <main className="pt-28 pb-24">
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          <div className="flex items-end justify-between gap-4 flex-wrap">
            <div>
              <span className="text-xs uppercase tracking-[0.2em] font-semibold text-[var(--forest)]">Finaliser l'achat</span>
              <h1 className="mt-2 font-display font-black text-4xl sm:text-5xl">Votre panier</h1>
            </div>
            {items.length > 0 && (
          ...
          {items.length === 0 ? (
            <div className="mt-12 rounded-3xl border border-dashed border-foreground/15 p-12 text-center">
              ...
              <h2 className="mt-5 font-display font-bold text-2xl">Votre panier est vide</h2>
              <p className="mt-2 text-foreground/60">Explorez la place de marché et trouvez votre camping-car de rêve.</p>
              <Link to="/buy" className="mt-6 inline-flex items-center gap-2 px-5 py-3 rounded-full bg-foreground text-background font-semibold">
                Acheter des vans <ArrowRight className="h-4 w-4" />
              </Link>
            </div>
          ) : (
            <div className="mt-10 grid lg:grid-cols-[1.5fr_1fr] gap-8">
                ...
                {items.map((i) => {
                  ...
                  return (
                    <li key={i.id} className="flex gap-4 p-4 rounded-3xl bg-white border border-black/5 shadow-soft">
                      ...
                      >
                        <img src={i.img} alt={i.name} className="absolute inset-0 h-full w-full object-cover" loading="lazy" />
                      </Link>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <button onClick={() => remove(i.id)} aria-label="Retirer" className="text-foreground/40 hover:text-foreground transition">
                            <Trash2 className="h-4 w-4" />
                          </button>
                        </div>
                        <div className="mt-3 flex items-end justify-between gap-3 flex-wrap">
                          <div className="inline-flex items-center gap-2 text-sm">
                            <span className="text-foreground/60">Qté:</span>
                            <button
              ...
              <aside className="h-fit rounded-3xl bg-white border border-black/5 shadow-soft p-6 sticky top-28">
                <h2 className="font-display font-black text-xl">Récapitulatif de la commande</h2>
                <dl className="mt-5 space-y-2 text-sm">
                  <div className="flex justify-between"><dt className="text-foreground/70">Sous-total</dt><dd className="font-semibold">{subtotal.toLocaleString()} €</dd></div>
                  <div className="flex justify-between"><dt className="text-foreground/70">Livraison & inspection</dt><dd className="font-semibold">{fees} €</dd></div>
                ...
                <button className="mt-6 w-full inline-flex items-center justify-center gap-2 px-5 py-3.5 rounded-full bg-foreground text-background font-semibold hover:opacity-90 transition">
                  Finaliser l'achat <ArrowRight className="h-4 w-4" />
                </button>
                <p className="mt-3 inline-flex items-center gap-2 text-xs text-foreground/60">
                  <ShieldCheck className="h-3.5 w-3.5 text-[var(--forest)]" /> Paiement sécurisé · Garantie satisfait ou remboursé 7 jours
                </p>
              </aside>
            </div>
          )}
